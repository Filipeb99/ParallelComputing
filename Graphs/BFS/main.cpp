...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        ...
        std::cerr << "  Run with a certain number of threads (no correctness run): <path/to/graph/file> <num_threads>" << std::endl;
        exit(1);
    }
    ...
    //Solution sphere
    solution sol_gold;
    sol_gold.distances = (int*)malloc(sizeof(int) * g->num_nodes);
    ...
    std::string sol_file = gdir.append("/solutions").append(gfile).append(".bfsol").c_str();
    load_solution_binary_int(sol_file.c_str(),sol_gold.distances);
    ...
    if (thread_count <= -1) {
        ...
        for (int i = 0; i < n_usage; i++) {
            ...
            for (int j=0; j<g->num_nodes; j++) {
                if (sol1.distances[j] != sol_gold.distances[j]) {
                    fprintf(stderr, "*** Results disagree at %d: %d, %d\n", j, sol1.distances[j], sol_gold.distances[j]);
                    tds_check = false;
                    break;
                }
            }
            ...
            for (int j=0; j<g->num_nodes; j++) {
                if (sol2.distances[j] != sol_gold.distances[j]) {
                    fprintf(stderr, "*** Results disagree at %d: %d, %d\n", j, sol2.distances[j], sol_gold.distances[j]);
                    bus_check = false;
                    break;
                }
            }
            ...
            for (int j=0; j<g->num_nodes; j++) {
                if (sol3.distances[j] != sol_gold.distances[j]) {
                    fprintf(stderr, "*** Results disagree at %d: %d, %d\n", j, sol3.distances[j], sol_gold.distances[j]);
                    hs_check = false;
                    break;
                }
            }
